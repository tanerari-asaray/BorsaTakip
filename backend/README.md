...
BIST discovery uses the documented TradeWize PAY last-price/details endpoint with `all=true`; the backend no longer treats the old 20-symbol list as the BIST universe. VİOP discovery uses the documented VİOP last-price/details endpoint with `all=true`. TradeWize documents these price endpoints as one request per second per product group, so the backend keeps discovery separate from the heavier historical-bar scan.

The scanner accepts `market`, `assetType`, `timeframe`/ `interval`, `limit`, `offset`, `minScore` and `includeWatch`. The default scan timeframe is 5m. Historical bars are requested from the documented `/api/v1/market-data/bars` endpoint with `includeOpenBar=false`; live/open bars are a separate upstream capability.
...
This distinction is intentional. TradeWize documents that last-price timestamps are the original price-event time and that old prices may be returned when no current record is available. Live Borsa İstanbul data is a separately licensed data service.
